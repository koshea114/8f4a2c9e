...
from chicken_detector import CHICKEN_KEYWORDS, _is_chicken_label, count_chicken_detections


class ChickenDetectorCountTests(unittest.TestCase):
    def test_all_keywords_are_detected_as_chicken(self):
        for keyword in CHICKEN_KEYWORDS:
            self.assertTrue(_is_chicken_label(keyword))
    ...
    def test_empty_or_missing_labels(self):
        detections = [{"confidence": 0.9}, {"label": "  "}]
        self.assertEqual(count_chicken_detections(detections), 0)
